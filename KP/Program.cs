...
string[] LetterArray(string[] input)
{
    ...
    for (int i = 0; i < input.Length; i++)
    {
        ...
        if (temp.Length <= 3)
        {
            count++; // сколько слов 3 слова 
        }
    }
    string[] letArray = new string[count]; // перебираем первый массив записываем в новый массив 
    int index = 0;
    for (int i = 0; i < input.Length; i++)
    {
        string temp = string.Empty;
...
string PrintArray(string[] array)
{
    ...
    for (int i = 0; i < array.Length; i++)
    {
        output = output + array[i] + " "; //пустая строка выбираем массив - входит то, что выходит - выводим
    }
    return output;
}*/
